<?php
    ...
    $sql = 'SELECT runID, accountID, characterID, time, placeID, money, createdAt FROM run ORDER BY ' . $sort . ' ' . $order . ' LIMIT ' . $firstResult . ',' . $resultsPerPage;
    ...
    <section class = "container brand">
        <h1 class = "center white-text"> Submitted Runs </h1>
            <table class = "striped bordered white-text">
                <thead>
                    <tr>
                        <th><a class = "white-text" href = <?php reload($page, 'accountID', $order, $sort) ?>> Username </a> </th>
                        <th><a class = "white-text" href = <?php reload($page, 'characterID', $order, $sort) ?>> Character </a> </th>
                        <th><a class = "white-text" href = <?php reload($page, 'time', $order, $sort) ?>> Time </a> </th>
                        <th><a class = "white-text" href = <?php reload($page, 'placeID', $order, $sort) ?>> Furthest </a> </th>
                        <th><a class = "white-text" href = <?php reload($page, 'money', $order, $sort) ?>> Total Money </a> </th>
